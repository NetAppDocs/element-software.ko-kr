...
입력 매개변수가 없습니다. `GetRemoteLoggingHosts` .



== 반환 값

그만큼 `GetRemoteLoggingHosts` 이 메서드는 다음과 같은 반환 값을 갖습니다.
...
원격 호스트
 a| 
전달된 로깅 정보를 수신하도록 구성된 호스트의 IP 주소, FQDN 및 포트 정보를 나열합니다.
 a| 
