...
[role="lead"]
NetApp SolidFire 와 Element 소프트웨어를 주기적으로 업데이트하여 새로운 기능, 향상된 기능, 버그 수정을 제공합니다. Element 12.9은 최신 릴리스로 보안 및 시스템 구성 요소 업데이트, 운영 개선 사항, 해결된 문제가 포함되어 있습니다.


NOTE: 누적된 소프트웨어 및 펌웨어 업데이트는 스토리지 클러스터에서 실행되는 현재 Element 버전을 기반으로 하는 Element 12.9 업그레이드의 일부로 설치됩니다. 예를 들어, 클러스터가 현재 Element 12.5를 실행 중인 경우 Element 12.9로 직접 업그레이드하여 Element 12.7, 12.8, 12.9의 누적 업데이트를 받을 수 있습니다. 지원되는 업그레이드 경로에 대해서는 기술 자료 문서를 참조하세요. https://kb.netapp.com/Advice_and_Troubleshooting/Data_Storage_Software/Element_Software/What_is_the_upgrade_matrix_for_storage_clusters_running_NetApp_Element_software["NetApp Element 소프트웨어를 실행하는 스토리지 클러스터의 업그레이드 매트릭스는 무엇입니까"^] .



== 원소 12.9

Element 12.9의 새로운 기능에 대해 알아보세요.



=== "SetRemoteLoggingHosts"에서 FQDN 지원

Element 12.9부터 다음을 사용할 수 있습니다. `SetRemoteLoggingHosts` 스토리지 클러스터의 로깅 호스트 값으로 정규화된 도메인 이름(FQDN) 또는 IP 주소를 구성하는 방법입니다.  응답에서 FQDN 호스트 값을 볼 수 있습니다. `GetRemoteLoggingHosts` 방법.  Element 12.8 및 이전 버전의 경우 로깅 호스트 값으로 IP 주소만 구성할 수 있습니다.

https://docs.netapp.com/us-en/element-software/api/reference_element_api_loggingserver.html["로깅 호스트 구성에 대해 자세히 알아보세요"]



== 요소 12.8

Element 12.8의 새로운 기능에 대해 알아보세요.
...
Element 12.7의 새로운 기능에 대해 알아보세요.
...
Element 12.5의 새로운 기능에 대해 알아보세요.
