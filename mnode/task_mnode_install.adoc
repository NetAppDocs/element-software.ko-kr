...
이 절차를 수행하기 전에 에 대해 잘 알고 있어야 합니다 link:../concepts/concept_solidfire_concepts_volumes.html#persistent-volumes["영구 볼륨"] 그리고 사용 여부를 결정합니다. 영구 볼륨은 선택 사항이지만 가상 머신(VM)이 손실된 경우 관리 노드 구성 데이터 복구에 권장됩니다.
