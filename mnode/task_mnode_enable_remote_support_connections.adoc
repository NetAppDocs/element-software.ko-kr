...
---
= 원격 NetApp 지원 세션 시작(Element 12.3 및 이전)
:allow-uri-read: 


[role="lead"]
Element 12.3 이하를 실행 중이고 SolidFire All-Flash 스토리지 시스템에 대한 기술 지원이 필요한 경우 NetApp Support를 사용하여 시스템을 원격으로 연결할 수 있습니다. 세션을 시작하고 원격 액세스를 위해 NetApp Support는 사용자 환경에 대한 역방향 SSH(Secure Shell) 연결을 열 수 있습니다.
