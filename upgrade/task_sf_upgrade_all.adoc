...
summary: NetApp SolidFire All-Flash 스토리지 시스템과 모든 구성 요소를 업그레이드할 수 있습니다. 
keywords: netapp, solidfire, cluster, upgrade, element 
---
= Element 12.3.x를 위해 NetApp SolidFire All-Flash 스토리지 시스템을 업그레이드하십시오


[role="lead"]
모든 NetApp 스토리지 구성요소를 순차적으로 업그레이드하여 구축 후 SolidFire Element 스토리지 시스템을 최신 상태로 유지할 수 있습니다.

이러한 구성요소는 관리 서비스, HealthTools, NetApp 하이브리드 클라우드 제어(HCC), Element 소프트웨어, 관리 노드 및 (설치에 따라 다름) vCenter Server용 Element 플러그인을 포함합니다.
...
+
HCC를 사용한 업그레이드는 이전 서비스 번들 버전에서 사용할 수 없습니다.
