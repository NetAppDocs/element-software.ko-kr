...
NOTE: 관리 서비스 2.22.7에는 원격 플러그인이 포함된 VMware vCenter Server 5.0용 NetApp Element 플러그인이 포함되어 있습니다. Element 플러그인을 사용하는 경우, 로컬 플러그인 지원을 제거하는 VMware 지침을 준수하도록 버전 5.0으로 업그레이드해야 합니다. https://kb.vmware.com/s/article/87880["자세한 정보"^]. 플러그인을 사용하지 않는 경우 선택적 업그레이드입니다.

[role="tabbed-block"]
====
.Element vCenter Plug-in 5.0 이상
--
VMware vCenter Server 5.0용 Element 플러그인부터 이 업그레이드 절차에서는 다음과 같은 업그레이드 시나리오에 대해 설명합니다.
