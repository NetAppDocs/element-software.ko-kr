...
... 을 참조하십시오 <<Upgrading H610S storage nodes to Element 12.3.x (phase 2)>> 각 노드에 대해 프로세스를 완료합니다.
... 필요한 경우 재인증을 수행하고 * Put cer/upgrades/{upgrade eId} * 를 선택합니다.
... 체험하기 * 를 선택합니다.
... 이전 단계의 업그레이드 ID를 * upgrade eId * 에 입력합니다.
...
.단계
. 스토리지 업그레이드 패키지를 다운로드합니다. SolidFire 스토리지 시스템의 경우 Element 소프트웨어로 이동하십시오 https://mysupport.netapp.com/site/products/all/details/element-software/downloads-tab["다운로드 페이지"^] 최신 스토리지 노드 이미지를 관리 노드가 아닌 디바이스로 다운로드합니다.
+
...
.무엇을 &#8217;필요로 할거야
. SolidFire 스토리지 시스템의 경우 Element 소프트웨어로 이동하십시오 https://mysupport.netapp.com/site/products/all/details/element-software/downloads-tab["다운로드 페이지"^].
