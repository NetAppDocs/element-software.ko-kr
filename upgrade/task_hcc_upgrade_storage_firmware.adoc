...
| 관리 노드는 외부 연결이 없는 어두운 사이트 내에 있습니다.  a| 
.. 스토리지 펌웨어 업그레이드 패키지를 관리 노드에서 액세스할 수 있는 장치로 다운로드합니다. SolidFire 스토리지 시스템의 경우 Element 소프트웨어로 이동하십시오 https://mysupport.netapp.com/site/products/all/details/element-software/downloads-tab["다운로드 페이지"^] 최신 스토리지 펌웨어 이미지를 다운로드합니다.
