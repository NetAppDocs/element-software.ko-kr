...
[NOTE]
====
* 2023년 11월부터 NetApp Hybrid Cloud Control 또는 REST API를 사용하여 구성 요소 업그레이드를 시작할 수 없습니다. 서명 키 인증서(개인 및 공개)가 2023년 11월 5일에 만료되었기 때문입니다. 이 문제는 다음을 통해 해결할 수 있습니다.link:task_hcc_upgrade_management_node.html["관리 노드의 전체 업그레이드 수행"] NetApp Hybrid Cloud Control 또는 REST API를 사용하여 관리 서비스, Element 소프트웨어 및 스토리지 펌웨어를 업그레이드하기 전에 버전 12.8 이상으로 업그레이드하세요.
+
버전 12.8 이상으로 새 관리 노드를 배포하는 경우 NetApp Hybrid Cloud Control 또는 REST API를 사용하여 다음 순서대로 구성 요소 업그레이드를 수행할 수 있습니다.<<sys_upgrade,시스템 업그레이드 순서>> .
...
* link:task_hcc_upgrade_element_software.html["Element 소프트웨어"] 및 link:task_hcc_upgrade_storage_firmware.html#storage-firmware-upgrade["스토리지 펌웨어"] 업그레이드를 위해 충분한 시간을 확보하셨습니다. Element 소프트웨어 12.5 이상으로 업그레이드할 경우, 업그레이드 소요 시간은 Element 소프트웨어 버전 및 펌웨어 업데이트에 따라 달라집니다.
...
[IMPORTANT]
====
** 관리 서비스 2.27 번들에는 vCenter Server 5.5용 Element 플러그인이 포함되어 있으며, 이는 관리 노드 12.8 이상과만 호환됩니다. 관리 서비스 2.27로 업데이트하는 경우 업그레이드 순서를 변경하고 Element 12.9로 업그레이드한 _후에_ 관리 서비스 번들을 업데이트해야 관리 노드와 관리 서비스 간의 호환성을 확보할 수 있습니다.
** 관리 서비스 2.21.61~2.26.40으로 업데이트하는 경우 Element 12.9로 업그레이드하기 _전에_ 관리 서비스 번들을 업데이트해야 합니다.
